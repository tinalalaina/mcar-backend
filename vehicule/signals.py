...
@receiver(pre_save, sender=VehiclePhoto)
def vehiclephoto_pre_save(sender, instance, **kwargs):
    _cleanup_old_file(instance, "image", VehiclePhoto)


@receiver(pre_delete, sender=VehiclePhoto)
def vehiclephoto_pre_delete(sender, instance, **kwargs):
    if instance.image:
