...
def blogpost_pre_save(sender, instance, **kwargs):
    _cleanup_old_file(instance, "cover_image", BlogPost)


@receiver(pre_delete, sender=BlogPost)
def blogpost_pre_delete(sender, instance, **kwargs):
    if instance.cover_image:
        if hasattr(instance.cover_image, 'url'):
             delete_file(instance.cover_image.url)
        elif hasattr(instance.cover_image, 'path'):
             delete_file(instance.cover_image.path)
        else:
             delete_file(str(instance.cover_image))


@receiver(pre_save, sender=BlogSection)
def blogsection_pre_save(sender, instance, **kwargs):
    _cleanup_old_file(instance, "image", BlogSection)


@receiver(pre_delete, sender=BlogSection)
def blogsection_pre_delete(sender, instance, **kwargs):
    if instance.image:
