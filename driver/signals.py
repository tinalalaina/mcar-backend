...
def _cleanup_old_file(instance, field_name: str, model_class):
    ...
    # Si il y avait un fichier et qu'il est différent du nouveau (ou que le nouveau est vide)
    if old_file and old_file != new_file:
         # Note: si old_file existe, il a une propriété .path ou .url selon le storage
        if hasattr(old_file, 'url'):
             delete_file(old_file.url)
        elif hasattr(old_file, 'path'):
             delete_file(old_file.path)
        else:
             # Fallback sur string si jamais c'est stocké bizarrement (ce qui ne devrait pas arriver avec FileField)
...
def driver_pre_save(sender, instance, **kwargs):
    ...
    file_fields = ['profile_photo', 'license_photo', 'cin_recto', 'cin_verso']
    for field in file_fields:
        _cleanup_old_file(instance, field, Driver)

@receiver(pre_delete, sender=Driver)
def driver_pre_delete(sender, instance, **kwargs):
    # Suppression de tous les fichiers associés au driver
    file_fields = ['profile_photo', 'license_photo', 'cin_recto', 'cin_verso']
    for field in file_fields:
